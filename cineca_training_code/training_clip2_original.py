...
# Per-image global normalization - norm 1
def normalize(array):
    array = array.astype(np.float32)
    return (array - array.min()) / (array.max() - array.min())


# Per-image per-channel normalization - norm 2
...
# Global per-channel normalization with stats computed on train_df - norm 3
...
# Adapted classes Dataset and Datamodule to integrate norm 3
class EuroSATMSIDataset(Dataset):
...
def main():
    ...
    # 2. Load data module
    data_module = EuroSATDataModule(train_df, val_df, test_df, label2idx, batch_size, band_stats) # batch_size = 8, 16, 32, 64(not sure)
    ...
    checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(
        ...
        filename="clip-msi2-eurosat-norm3-{epoch:02d}-{val_acc:.4f}", # not a format string, values will be filled at runtime
