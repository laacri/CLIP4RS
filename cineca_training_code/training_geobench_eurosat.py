...
data_path = '../../m-eurosat/' # the script will run inside the thesis/ folder
...
def main():
    ...
    print("\nVALIDATION set: -------------------------------------------------------")
    val_df = pd.read_csv('../val_df_m-eurosat.csv')
    print(val_df.head())
    ...
    if args.model == 1:
        ...
        logger = CSVLogger(save_dir=log_dir, name="clip-msi1-geobench-eurosat")

        # define the logger object
        logger_tb = TensorBoardLogger(tb_log_dir, name = "clip-msi1-geobench-eurosat", log_graph = True)
        ...
        checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(
            dirpath=checkpoint_dir,
            filename="clip-msi2-geobench-eurosat-{epoch:02d}-{val_acc:.4f}", # not a format string, values will be filled at runtime
            save_top_k=1, # save only the checkpoint with the highest performance (here, val_acc)
            monitor="val_acc",
            mode="max",
            save_last = True
        ...
        # # 5. Specify logger in csv format
        logger = CSVLogger(log_dir, name="clip-msi2-geobench-eurosat")

        # define the logger object
        logger_tb = TensorBoardLogger(tb_log_dir, name = "clip-msi2-geobench-eurosat", log_graph = True)
